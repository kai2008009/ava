<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Select.ascx.cs" Inherits="AVA.ResourcesPlatform.AdminUI.Pub.RecordEquipment.Select1" %>
...
break;
    ...
    <tr class="trout">
        ...
        <td class="wd1 select" onclick="Select(&quot;<%= item.Guid.ToString(null).ToScriptTag()%>&quot;,&quot;<%= item.Name.ToString(null).ToScriptTag() %>&quot;);if (typeof (window.parent.window.GetTimeRange) != 'undefined') {window.parent.window.GetTimeRange(); }">
        </td>
        <%
            foreach (var field in this.Fields)
            {
