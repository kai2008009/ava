<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="AVA.ResourcesPlatform.AdminUI.Pub.RecordEquipment.List" %>
        ...
        <div class="gIbx">
            <div class="mContent">
                <div id="dvToolbar_inbox" class="g-toolbar">
                    <div class="btngrp">
                        <div style="margin-top: 5px;">
                            ...
                            <%= new HtmlHidden("$Field", "Status")%>
                            <%= new HtmlHidden("$FieldType", (int)Z.Core.NHibernateCore.Search.FieldTypeEnum._int)%>
                            <%= new HtmlHidden("$ExpressionKey", null)%>
                            <%= new HtmlHidden("$QueryType", (int)Z.Core.NHibernateCore.Search.QueryTypeEnum.Eq)%>
                            <%= new HtmlSelect("$FieldValue", "Value", "Key") { Option = typeof(AVA.ResourcesPlatform.Model.Pub.RecordEquipmentStatusEnum) }.InsertOption("::" +  DefineEnum.选择.Define() + "::", null, 0)%>
                    ...
                    <div class="btngrp btngrp-ext">
                        <div class="btngrp">
                            <div class="btn btn-dft btn-dft-gl btn-dft-icon" title="" onclick="GoAdd()">
                                <span>
                                    <%= DefineEnum.添_加.Define() %><b class="ico ico-add"></b></span>
                            </div>
                            <div class="btn btn-dft btn-dft-gc" title="" onclick="GoBatchDelete()">
                                <span>
                                    <%= DefineEnum.删_除.Define() %></span>
                            </div>
                            <div class="btn btn-dft btn-dft-gr btn-dft-pd" title="" onclick="GoBatchStatus()">
                                <span>
                                    更新状态</span>
                                </div>
                        </div>
                ...
                <div class="gIbx-tablayout">
                    <div class="txt-12 listContainer" id="list" style="overflow-y:hidden;overflow-x:auto; padding-bottom:10px">
                    </div>
                </div>
            </div>
        </div>
    ...
    <div id="divViewDefine" class="gSys-msg-win" style="display: none; right: 10px; top: 45px;">
        ...
        <div class="cont bdr-c-dark">
            <div class="gSys-inner-dict">
                <form id="frmViewDefine" action="">
                <div class="result bdr-c-mid">
                    <div class="inside">
                        <%= new HtmlHidden("KEY",KEY) %>
                        <ul id="ulViewDefine">
                            <%
                                Type t = typeof(AVA.ResourcesPlatform.AdminUI.Pub.RecordEquipment.List1);
                                var list = UserControlDefine.GetListViewDefine(UserCode, t.FullName, AVA.ResourcesPlatform.AdminUI.Pub.RecordEquipment.List1.LISTRPT);
        ...
        <div class="cont bdr-c-dark">
            <div class="gSys-inner-dict">
                ...
                <div class="ft bdr-c-dark bg-cont">
                    ...
                    <div class="opt">
                        <%= new HtmlSubmit("Send", null, "AdvancedSearch(this);") { Class = "hidden" }%>
...
<script type="text/javascript">
    ...
    function AddQueryParameter(send) {
        Global.HtmlControl("/Pub/RecordEquipment/QueryParameterContainer", null, send, function (result) {
            $(send).parent().parent().after(result.Html);
        });
    }

    function GetQueryParamter(send) {
        Global.HtmlControl("/Pub/RecordEquipment/QueryParameter", "Key=" + encodeURIComponent(send.value), send, function (result) {
            ...
            if (dicKey != "") {
                Global.HtmlControl("/Pub/RecordEquipment/QueryParameterDictionary", "Key=" + encodeURIComponent(dicKey), send, function (resultDictionary) {
...
<script type="text/javascript">
    ...
    function Search(send) {
        ...
        Global.UserControl("/Pub/RecordEquipment/List", params, send, function (result) {
    ...
    function Delete(send) {
        var key = send.getAttribute("key");
        RecordEquipmentAjax.Delete(key, send, function () {
            Search();
        });
    };

    //批量删除
    function BatchDelete(send, key) {
        RecordEquipmentAjax.BatchDelete(key, null, function () {
            Search();
        });
    };

    function GoBatchStatus()
    {
        $(".spnRecondEquipmentControler").click();
    }
</script>
<script type="text/javascript">
    //初始方法
