<%@ Control Language="C#" Inherits="AVA.ResourcesPlatform.WebUI.Control.My.TimeRange" %>
<%
...
%>
<label>
    <%= new HtmlCheckBox("", false, "", "") { Disabled = true }.Style(new { Width = "Auto", Display="inline" })%>
...
%>
<label title="">
    <%= new HtmlCheckBox("TimeRange", false, startTime.ToLongDate() + "|" + endTime.ToLongDate(), "").Style(new { Width = "Auto", Display="inline" })%>
    <%= item.StartTime.ToTime()%>
    -
    <%= item.EndTime.ToTime()%>
    (<%= item.Chatper %>)
